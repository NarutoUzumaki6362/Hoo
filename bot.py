...
# Bot Configuration
TELEGRAM_BOT_TOKEN = '8062201499:AAEj0Z13mMlJJF0NiHXKVWXtXFTfdyYhUyY'
OWNER_ID = 6710024903  # 👈 Put your real Telegram user ID here
CO_OWNERS = []  # List of user IDs for co-owners
OWNER_CONTACT = "Contact to buy keys"
ALLOWED_GROUP_IDS = [-1002228518289]
max_duration = 120
bot_open = False
OWNER_USERNAME = "@SPIDY_OWNEROP"
...
START_IMAGES = [
    {
        'url': 'https://ik.imagekit.io/6ilngyaqa/1747718670130-1000011586_0CCMJGBBT.jpg',
        'caption': (
            '''✨ *Welcome to the Ultimate Attack System Bot!* ✨

🚀 *Get ready to simulate high-speed load attacks!*

📌 *Usage Example:* `20.235.43.9 14533 120 100`

💡 *Note:* Use `/attack` to start. Keys are needed for full access.

🔗 *Join our channel:* https://t.me/your_channel_here

👑 *Bot By:* @SPIDY_OWNEROP'''
        )
    },
...
async def owner_settings(update: Update, context: CallbackContext):
    if not is_owner(update):
        await update.message.reply_text("❌ *⚠️ *Only the owner can access these settings!**", parse_mode='Markdown')
        return
...
async def set_vps_count(update: Update, context: CallbackContext):
    ...
    await update.message.reply_text(
        f"⚠️ ⚙️ *Enter number of VPS to use* (current: {ACTIVE_VPS_COUNT}, available: {len(VPS_LIST)}):",
        parse_mode='Markdown'
    )
    ...
    if chat.type == "private":
        if not is_authorized_user(update):
            await update.message.reply_photo(
                photo=image['url'],
                caption=f"❌ *🚫 *This bot is not authorized here.**\n\n",
                parse_mode='Markdown'
            )
            return
...
async def generate_special_key_duration(update: Update, context: CallbackContext):
    try:
        ...
        await update.message.reply_text(
            "⚠️ *Enter the custom format for the special key (e.g., 'SPIDEY-OFFICIAL-KEY' will create key 'SPIDEY-OFFICIAL-KEY-XXXX'):*",
            parse_mode='Markdown'
        )
...
async def generate_special_key_format(update: Update, context: CallbackContext):
    ...
    await update.message.reply_text(
        ...
        f"⚠️ *This key provides enhanced attack capabilities when you [removed] [removed]!*",
        parse_mode='Markdown'
    )
    return ConversationHandler.END
...
async def redeem_key_input(update: Update, context: CallbackContext):
    ...
    if key in keys and keys[key]['expiration_time'] > time.time():
        ...
        text = (
            ...
            f"👑 *Bot Owner:* {current_display_name}\n\n"
            f"⚡ *Happy attacking and Raja ki [removed] phaad do!*"
        )
        escaped_text = escape_markdown(text, version=2)
        await update.message.reply_text(
            escaped_text,
...
async def attack_start(update: Update, context: CallbackContext):
    ...
    if chat.type == "private":
        if not is_authorized_user(update):
            await update.message.reply_text("❌ *🚫 *This bot is not authorized here.**", parse_mode='Markdown')
            return ConversationHandler.END
...
async def attack_input(update: Update, context: CallbackContext):
    ...
    attack_type = "⚡ *SPECIAL ATTACK* ⚡" if is_special else "⚔️ *⚔️ *Attack Started!*!*"
    current_display_name = get_display_name_from_update(update)
    ...
    try:
        ...
        logging.error(f"Error starting attack 
        await update.message.reply_text(
...
async def set_threads_start(update: Update, context: CallbackContext):
    if not (is_owner(update) or is_co_owner(update)):
        await update.message.reply_text("❌ *Only the owner or co-owners can set max 
        return ConversationHandler.END

    await update.message.reply_text("⚠️ *Enter the maximum number of 
...
async def rules(update: Update, context: CallbackContext):
    ...
    rules_text = (
        ...
        "[removed] RULES FOLLOW KRNA WARNA GND MAR DUNGA.\n\n"
        "JO BHI Raja KI MAKI [removed] PHAADKE SS DEGA USSE EXTRA KEY DUNGA.\n\n"
    )
...
application = Application.builder().token("8062201499:AAEj0Z13mMlJJF0NiHXKVWXtXFTfdyYhUyY").build()
